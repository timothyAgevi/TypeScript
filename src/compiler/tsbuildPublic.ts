...
function createBuildOrUpdateInvalidedProject<T extends BuilderProgram>(
    ...
    function emit(
        ...
        if (
            ...
            // TODO: sheetal - noEmitOnError with --outFile and buildInfo untangling
...
function getUpToDateStatusWorker<T extends BuilderProgram>(state: SolutionBuilderState<T>, project: ParsedCommandLine, resolvedPath: ResolvedConfigFilePath): UpToDateStatus {
    ...
    if (buildInfoPath) { // TODO:: sheetal Handling without incremental
        ...
        if (buildInfo.program) {
            ...
            if (
                (!project.options.noEmit && (buildInfo.program as ProgramMultiFileEmitBuildInfo).emitDiagnosticsPerFile?.length) ||
            ...
            // TODO:: sheetal : can we have noEmitOnError on commandLine to change? - is in buildinfo
            // Tests with and without incremetnal and --out
            // Handle semantic diagnostics --noCheck on commandLine and in buildInfo
...
function reportUpToDateStatus<T extends BuilderProgram>(state: SolutionBuilderState<T>, configFileName: string, status: UpToDateStatus) {
    switch (status.type) {
        ...
        case UpToDateStatusType.OutOfDateBuildInfoWithErrors:
            return reportStatus(
                state,
                Diagnostics.Project_0_is_out_of_date_because_buildinfo_file_1_indicates_that_program_had_errors,
                relName(state, configFileName),
                relName(state, status.buildInfoFile),
            );
