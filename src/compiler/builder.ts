...
function createBuilderProgramState(newProgram: Program, oldState: Readonly<ReusableBuilderProgramState> | undefined): BuilderProgramState {
    ...
    state.hasSyntaxErrors = !!state.program.getConfigFileParsingDiagnostics().length ||
    ...
    if (useOldState && (oldState!.hasSyntaxErrors && !state.hasSyntaxErrors)) {
        // If we removed syntax errors, we need to emit buildInfo again
        state.buildInfoEmitPending = true;
    }
    return state;
}
...
function getBuildInfo(state: BuilderProgramState): BuildInfo {
    Debug.assertIsDefined(state.program);
    ...
    // TODO:: Check for DeclarationErrors
    if (!isIncrementalCompilation(state.compilerOptions)) return createBuildInfo(/*program*/ undefined, state.hasSyntaxErrors || hasSemanticErrors());
    ...
    if (state.compilerOptions.outFile) {
        ...
        // TODO:: sheetal cache semantic diagnostics and emit dts diagnostics
        return createBuildInfo(program, state.hasSyntaxErrors || hasSemanticErrors());
    }
    ...
    let buildInfoRecordedErrors: boolean | undefined;
    ...
    Debug.assert(!state.changedFilesSet.size);
    ...
    return createBuildInfo(program, hasErrorsProgramMultiFileEmitBuildInfo());

    function hasSemanticErrors() {
        return !!forEach(state.program!.getSourceFiles(), f => getSemanticDiagnosticsOfFile(state, f).length);
    }

    function hasErrorsProgramMultiFileEmitBuildInfo() {
        // Record hasErrors only if buildInfo doesnt contain any error record
        return !buildInfoRecordedErrors ?
            state.hasSyntaxErrors || !!forEach(state.program!.getSourceFiles(), f => state.program!.getProgramDiagnostics(f).length) :
            undefined;
    }
    ...
    function convertToProgramBuildInfoDiagnostics(diagnostics: Map<Path, readonly Diagnostic[]> | undefined) {
        ...
        if (diagnostics) {
            for (const key of arrayFrom(diagnostics.keys()).sort(compareStringsCaseSensitive)) {
                ...
                buildInfoRecordedErrors ||= !!value.length;
            }
        }
        return result;
    }
...
export function createBuilderProgramUsingProgramBuildInfo(buildInfo: BuildInfo, buildInfoPath: string, host: ReadBuildProgramHost): EmitAndSemanticDiagnosticsBuilderProgram {
    ...
    if (isProgramBundleEmitBuildInfo(program)) {
        ...
        state = {
            ...
            hasSyntaxErrors: buildInfo.hasErrors,
        };
    }
    else {
